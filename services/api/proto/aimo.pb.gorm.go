...
func DefaultReadUserInfo(ctx context.Context, in *UserInfo, db *gorm.DB) (*UserInfo, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(UserInfoORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &UserInfoORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(UserInfoORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormResponse).(UserInfoORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type UserInfoORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteUserInfo(ctx context.Context, in *UserInfo, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(UserInfoORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&UserInfoORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(UserInfoORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type UserInfoORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteUserInfoSet(ctx context.Context, in []*UserInfo, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&UserInfoORM{})).(UserInfoORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&UserInfoORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&UserInfoORM{})).(UserInfoORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type UserInfoORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*UserInfo, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*UserInfo, *gorm.DB) error
}

// DefaultStrictUpdateUserInfo clears / replaces / appends first level 1:many children and then executes a gorm update call
...
type UserInfoORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserInfoORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchUserInfo executes a basic gorm update call with patch behavior
...
type UserInfoWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *UserInfo, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type UserInfoWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *UserInfo, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type UserInfoWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *UserInfo, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type UserInfoWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *UserInfo, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetUserInfo executes a bulk gorm update call with patch behavior
